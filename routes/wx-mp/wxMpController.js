/*
 * @Author: your name
 * @Date: 2022-04-19 11:32:27
 * @LastEditTime: 2022-09-30 18:17:21
 * @LastEditors: 陈瑞峰 chenryuf@163.com
 * @Description: 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 * @FilePath: \fztx\routes\wx-mp\wxMpController.js
 */
...
var Mpush = require('./mp_push');
var siteWebUrl = '福建省事业单位公开招聘考试报名平台http://220.160.53.33:8903'
var siteWebName = '福建省事业单位公开招聘考试报名平台'
var secretOrPrivateKey = "wxMpReply"; // 这是加密的key（密钥）

var picHost = process.env.PIC_HOST;
//推送服务
...
router.get("/ZX", async (req, res) => {
    ...
    _.each(datas, (n) => {
        ...
        backList.push({
            title: n.title,
            detailUrl: n.url || `${host}/mp/ZDE?newId=${n.newId}`,
            noticeTime: WxDbHelper.changeDate(noticeTime, "yyyy/MM/dd"),
            areaCode: n.areaCode,
            proTitle: n.proTitle,
            areaName: (areaCodeToName[n.areaCode] || {}).name || "",
    ...
    _.each(datas2, (n) => {
        ...
        backList2.push({
            title: n.new_title,
            detailUrl: n.new_url || `${host}/mp/ZDE?newId=${n.new_id}`,
            noticeTime: WxDbHelper.changeDate(noticeTime, "yyyy/MM/dd"),
            areaCode: n.areaCode,
...
router.get("/ZXS", async (req, res) => {
    ...
    var datas2 = await WxDbHelper.getSZPrj2(keyword, year, areaCode);
    var datas3 = await WxDbHelper.getSZPrj3(keyword, year, areaCode);
    // console.log(datas.length,'data1');
    // console.log(datas2.length,'data2');
    // console.log(datas3.length,'data3');
    var backList = [];
    _.each(datas, (n) => {
        var noticeTime = n.noticeTime;
