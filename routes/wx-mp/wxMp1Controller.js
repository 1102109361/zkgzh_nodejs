...
var Mpush = require('./mp_push1');
var siteWebUrl = '福建省国有企业公开招聘考试报名平台http://220.160.53.33:8904'
var siteWebName = '福建省国有企业公开招聘考试报名平台'

var secretOrPrivateKey = "wxMpReply"; // 这是加密的key（密钥）

var picHost = process.env.PIC_HOST1;
//推送服务
...
router.post("/QRCode", async (req, res) => {
    ...
    var data = {
        ...
        "action_info": {
            "scene": {
                "scene_str": "BIND_GQ_" + params.idCard
            }
        }
    };
...
router.get("/ZX", async (req, res) => {
    ...
    _.each(datas, (n) => {
        ...
        backList.push({
            title: n.title,
            detailUrl: n.url || `${host}/mp1/ZDE?newId=${n.newId}`,
            noticeTime: WxDbHelper.changeDate(noticeTime, "yyyy/MM/dd"),
            areaCode: n.areaCode,
            proTitle: n.proTitle,
            areaName: (areaCodeToName[n.areaCode] || {}).name || "",
    ...
    _.each(datas2, (n) => {
        ...
        backList2.push({
            title: n.new_title,
            detailUrl: n.new_url || `${host}/mp1/ZDE?newId=${n.new_id}`,
            noticeTime: WxDbHelper.changeDate(noticeTime, "yyyy/MM/dd"),
            areaCode: n.areaCode,
