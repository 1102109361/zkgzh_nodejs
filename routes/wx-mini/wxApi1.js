...
var wx_mini = JSON.parse(process.env.WX_MINI1 || '{}');
...
router.post("/requestKSBMServe", async (req, res) => {
    ...
    console.error("requestKSBMServe1_调用日志：", JSON.stringify(params));
    if (!requestUrl) {
    ...
    if (files) {
        ...
        request.post(requestUrl, {
            ...
            if (error || response.statusCode != 200) {
                console.error("requestKSBMServe10_error: requestUrl=", requestUrl, '|error:', error);
                res.json({
                    code: -1,
                    msg: "请求失败，服务出错",
                    data: error
