/*
 * @Author: your name
 * @Date: 2022-04-28 13:54:31
 * @LastEditTime: 2022-08-03 13:11:53
 * @LastEditors: 陈瑞峰 chenryuf@163.com
 * @Description: 打开koroFileHeader查看配置 进行设置: https://github.com/OBKoro1/koro1FileHeader/wiki/%E9%85%8D%E7%BD%AE
 * @FilePath: \fztx\routes\wx-mini\wxApi.js
 */
...
var wx_mini = JSON.parse(process.env.WX_MINI || '{}');
...
router.post("/requestKSBMServe", async (req, res) => {
    ...
    console.error("requestKSBMServe_调用日志：", JSON.stringify(params));
    if (!requestUrl) {
    ...
    if (files) {
        ...
        request.post(requestUrl, {
            ...
            if (error || response.statusCode != 200) {
                console.error("requestKSBMServe_file_error", error);
                res.json({
                    code: -1,
                    msg: "请求失败，服务出错",
                    data: error
    ...
    request({
        ...
        if (error || response.statusCode != 200) {
            console.error("requestKSBMServe_error: requestUrl=", requestUrl, '|error:', error);
            res.json({
                code: -1,
                msg: "请求失败，服务出错",
                data: error
