...
async function set(key, data) {
    ...
    var redisData = await DbTools2.queryAsync(sql1);
    if (redisData.length) {
        ...
        return await DbTools2.queryAsync(sql);
    } else {
        let params = {
            key,
            data,
...
async function get(key) {
    ...
    var data = await DbTools2.queryAsync(sql);
...
async function keys(key, cb) {
    ...
    try {
        let data = await DbTools2.queryAsync(sql);
        cb(data)
    } catch (error) {
        cb(error)
    }
...
async function expire(key, time) {
    ...
    var data = await DbTools2.queryAsync(sql1)
