...
async function set(key, data) {
    ...
    var redisData = await DbTools3.queryAsync(sql1);
    if (redisData.length) {
        ...
        return await DbTools3.queryAsync(sql);
    } else {
        let params = {
            key,
            data,
...
async function get(key) {
    ...
    var data = await DbTools3.queryAsync(sql);
...
async function keys(key, cb) {
    ...
    try {
        let data = await DbTools3.queryAsync(sql);
        cb(data)
    } catch (error) {
        cb(error)
    }
...
async function expire(key, time) {
    ...
    var data = await DbTools3.queryAsync(sql1)
